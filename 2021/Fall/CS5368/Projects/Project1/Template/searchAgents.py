...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # 初始状态 是一个位置和一个已访问的角落的列表  方便做目标测试的
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # 只要不是墙 这个状态就是满足的 放入结果列表
            # 这里的状态要与我们定义的状态一致 满足这个就好了的
            x, y = state[0]
            ...
            if not hitswall:
                ...
                if node in self.corners:
                    if node not in visitedCornersList:
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # print('\n'.join(['%s:%s' % item for item in problem.__dict__.items()]))
    # position, foodGrid = state
    # walls
    # startingGameState
    # print(position)
    ...
    # 这一题写的和上一题的启发函数一样 因为其实这种思路是通用的欸
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        isGoal =self.food.asList().count((x,y)) == 1
        return isGoal
        util.raiseNotDefined()
